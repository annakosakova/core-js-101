...
function angleBetweenClockHands(date) {
  ...
  const currHour = 0.5 * (60 * hour + minute);
  const currMinute = 6 * minute;
  const difference = currHour - currMinute > 180 ? currHour - currMinute - 180
    : currHour - currMinute;
  return (Math.abs(difference) * Math.PI) / 180;
}
